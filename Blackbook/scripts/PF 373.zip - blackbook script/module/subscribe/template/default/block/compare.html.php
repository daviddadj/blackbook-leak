		...
		<table id="tbl_compare" cellpadding="0" cellspacing="0">
			...
			<tr id="trHeader">
				...
				{foreach name=iteTitle from=$aPackages.packages item=aPackage}
					<th class="th_package_title {if is_int($phpfox.iteration.iteTitle/2)}thTitleEven{else}thTitleOdd{/if}"{if $bIsDisplay && !empty($aPackage.background_color)} style="background-color:{$aPackage.background_color}"{/if}>{$aPackage.title}</th>
				{/foreach}
			</tr>
			{if $bIsDisplay}
			<tr class="trDescription">
				...
				{foreach name=iteDescription from=$aPackages.packages item=aPackage}
					<td class="tdPackageDescription {if is_int($phpfox.iteration.iteDescription/2)}tdDescriptionEven{else}tdDescriptionOdd{/if}"{if $bIsDisplay && !empty($aPackage.background_color)} style="background-color:{$aPackage.background_color}"{/if}>{$aPackage.description}</td>
				{/foreach}
			</tr>
